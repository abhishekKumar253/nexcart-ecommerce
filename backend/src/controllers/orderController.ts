...
export async function listOrders(
  ...
  try {
    ...
    const previewByOrder: Record<string, PreviewItem[]> = {};
    ...
    if (orderIds.length > 0) {
      ...
      for (const row of itemRows) {
        const list = previewByOrder[row.orderId] ?? [];
        list.push({
          name: row.name,
          slug: row.slug,
        ...
        previewByOrder[row.orderId] = list;
      }
    }

    const ordersPayload = rows.map((o) => ({
      ...o,
      previewItems: previewByOrder[o.id] ?? [],
    }));
...
export async function createVideoInvite(
  ...
  try {
    ...
    const joinUrl = `${env.FRONTEND_URL.replace(/\/+$/, "")}/orders/${
      order.id
    }/call`;
